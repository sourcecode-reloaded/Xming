    ...
    int	    addrlen)
{
	...
	if (fromAddr && memcmp(regAddr, fromAddr, regAddrlen) != 0) {
	    ErrorF("XdmcpRegisterConnection: fromAddr != regAddr %d.%d.%d.%d\n", (CARD8)address[0], (CARD8)address[1], (CARD8)address[2], (CARD8)address[3]);
	    memcpy(regAddr, fromAddr, regAddrlen);
	}
    }
    ...
    ErrorF("XdmcpRegisterConnection: newAddress %d.%d.%d.%d\n", newAddress[0], newAddress[1], newAddress[2], newAddress[3]);
