...
Win32System(const char *cmdline)
{
    ...
    /* For Xming build, safety tests */
    if (cmdline==NULL)
    {
	ErrorF("Win32System called with null pointer!\n");
	return -1;
    }
    if (!strstr(cmdline,"xkbcomp"))
    {
	ErrorF("Win32System command '%s' incorrect!\n", cmdline);
	return -1;
    }
#endif
    STARTUPINFO si;
...
char tmpname[PATH_MAX];
...
#endif
	{
		...
		nameRtrn[nameRtrnLen-1]= '\0';
            ...
            if (buf != NULL)
#if defined WIN32 && defined __MINGW32__
        	/* For Xming build, remove the temporary file */
        	unlink(tmpname);
#endif
                xfree (buf);
	    return True;
